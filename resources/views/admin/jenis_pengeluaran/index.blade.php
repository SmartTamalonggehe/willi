...
@section('judul','Jenis Pengeluaran')

@section('jenisPengeluaran','menu-item-active')
		...
		<div class="card card-custom gutter-b">
			<div class="card-header flex-wrap border-0 pt-6 pb-0">
				<div class="card-title">
					<h3 class="card-label">@yield('judul')
					<span class="d-block text-muted pt-2 font-size-sm">Klik 2x untuk mengubah atau menghapus data.</span></h3>
				</div>
...
@include('admin.jenis_pengeluaran.form')
	...
	<script>  
		...
		$(document).ready(function () {
			$("#formKu").on('submit',function(e){
			  ...
			  if (save_method=="add") { 
				  url="{{ route('jenisPengeluaran.store') }}"
				  method="POST"
			  } else {
				  url="jenisPengeluaran/"+id
				  method="PUT"
			  }
			  $.ajax({
			  url: url,
